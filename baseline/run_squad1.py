...
def main():
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_iter):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    optimizer.zero_grad()
                    global_step += 1
                    # 每1000个step测试一次 model在验证集上的F1,如果优于best_f1，保存该模型
                    if global_step % args.save_checkpoint_steps == 0:
                        ...
                        print(scores)
                        exact_match = float(scores.split(":")[1].split("\"")[1])
                        # f1 = float(scores.split(":")[2].split("}")[0])
                        f1 = float(scores.split(":")[2].split("\"")[1])
                        if f1>best_F1:
                            logger.info("Saving model with best F1: %.3f -> %.3f on epoch=%d" % (best_F1, f1, epoch))
            ...
            if step % args.log_freq == 0:
                logger.info("step:({0},{1}) ,step_loss:{2},learning_rate:{3},".format(epoch, global_step,final_loss,optimizer.param_groups[0]['lr']))
    ...
    if args.do_train:
        ...
        if args.max_steps == -1:
            ...
            logger.info( "training_sequences_per_second:{0}" .format(len(train_features) * args.num_train_epochs / time_to_train))
    ...
    # if args.mode=="eval" :
        # logger.info("exact_match:{0},F1:{1}" .format(exact_match,f1))
