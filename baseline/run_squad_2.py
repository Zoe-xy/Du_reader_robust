...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            start_indexes = _get_best_indices(result.start_logits, n_best_size)
            end_indexes = _get_best_indices(result.end_logits, n_best_size)
...
def get_answers(examples, features, results, args):
    ...
    for ex, feat, result in match_results(examples, features, results):
        # 更改args.n_best_size
        best_size = 100
        start_indices = _get_best_indices(result.start_logits, len(result.start_logits))
        end_indices = _get_best_indices(result.end_logits, len(result.end_logits))
...
def main():
    ...
    parser.add_argument("--mode",
                        default="",
                        type=str,
                        help="decide whether to predict or eval"
                        )
    parser.add_argument("--save_checkpoint_steps",
                        type=int,
                        default=2000,
                        help="the number of steps that compare model and save checkpoint"
                        )
    parser.add_argument("--max_checkpoint",
                        type=int,
                        default=5,
                        help="max checkpoint to save"
                        )
    parser.add_argument("--da_file",
                        default=None,
                        type=str,
                        help="data_augamentation dataset"
                        )
    ...
    if args.do_train:
        ...
        try:
            ...
            if not args.skip_cache:
                ...
                logger.info("Cached_train features_file" + cached_train_features_file)
    ...
    if args.do_train:
        ...
        logger.info("train_start:True")
        logger.info("training_samples: %d " % len(train_examples))
        logger.info("training_features: %d " % len(train_features))
        logger.info("train_batch_szie:%d"%(args.train_batch_size))
        logger.info("steps:%d " % num_train_optimization_steps)
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_iter):
                ...
                start_loss = loss_fct(start_logits, start_positions.cuda())
                end_loss = loss_fct(end_logits, end_positions.cuda())
                ...
                if step % args.log_freq == 0:
                    logger.info(
                        # "step:({0},{1}) ,step_loss:{2},learning_rate:{3},".format(epoch, global_step, final_loss,
                ...
                if (step+1) % args.save_checkpoint_steps==0:
                    ...
                    num_checkpoint=num_checkpoint % args.max_checkpoint
                    output_model_file = os.path.join(args.output_dir, "pytorch_model%d.bin"%(num_checkpoint))
                    logger.info("save checkpoint in step,epoch({0},{1}),number {2}".format(step,epoch,num_checkpoint))
                    # 存时有"model"
                    torch.save({"model": model_to_save.state_dict()}, output_model_file)
                    output_config_file = os.path.join(args.output_dir,"bert_config%d.json"%(num_checkpoint) )
    ...
    # if args.do_train and not args.skip_checkpoint:
    ...
    if (args.mode == "eval" or args.mode == "predict") and (args.local_rank == -1):
        ...
        if args.mode == "predict":
            ...
            eval_examples = read_squad_examples(
                input_file=args.eval_file, is_training=False, version_2_with_negative=args.version_2_with_negative)
        else:
            raise ValueError(
                "mode must be predict or eval")
        ...
        logger.info("eval_samples :%d" % len(eval_examples))
        logger.info("eval_features:%d" % len(eval_features))
        ...
        if args.mode == "predict":
            ...
            output_prediction_file = os.path.join(args.output_dir, "eval.json")
            output_nbest_file = os.path.join(args.output_dir, "nbest_eval.json")
        else:
            raise
            ValueError(
                "mode must be predict or eval")
        ...
        #     f.write(json.dumps(answers,ensure_ascii=False, indent=4) + "\n")
        # with open(output_nbest_file, "w",encoding='utf-8') as f:
        #     f.write(json.dumps(nbest_answers,ensure_ascii=False, indent=4) + "\n")
        ...
        # if args.mode=="eval" and is_main_process():
        if args.mode == "eval":
            import sys
            import subprocess
            eval_out = subprocess.check_output([sys.executable, args.eval_script,
                                                args.eval_file, args.output_dir + "/eval.json"])
            # 移除首尾
            scores = str(eval_out).strip()
            print(args.init_checkpoint)
            print(scores)
            f1 = float(scores.split(":")[1].split("\"")[1])
            # f1 = float(scores.split(":")[2].split("}")[0])
            exact_match = float(scores.split(":")[2].split("\"")[1])
    ...
    if args.do_train:
        ...
        if args.max_steps == -1:
            logger.info("e2e_train_time:{0}".format(time_to_train))
            logger.info(
                "training_sequences_per_second:{0}".format(len(train_features) * args.num_train_epochs / time_to_train))

            logger.info("final_loss: %.2f" % (final_loss))
        else:
            logger.info("e2e_train_time:{0}".format(time_to_train))
            logger.info("training_sequences_per_second:%.2f" % (args.train_batch_size * args.gradient_accumulation_steps \
                                                                * args.max_steps * gpu_count / time_to_train))
            logger.info("final_loss: %.3f" % (final_loss))
    if args.do_predict:
        logger.info("e2e_inference_time:{0}".format(time_to_infer))
        logger.info("inference_sequences_per_second：%.2f" % (len(eval_features) / time_to_infer))
    if args.mode=="eval" :
        logger.info("F1:{0},exact_match:{1}" .format(f1,exact_match))
